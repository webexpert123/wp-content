	...
	<tr>
		...
		<td class="order-data">
			<table>
				...
				<tr class="order-date">
					<th><?php $this->order_date_title(); ?>1</th>
		...
		<tr>
			<?php
				foreach ( $headers as $column_class => $column_title ) {
					if($column_title=="Quantity"){continue;}
			...
			<tr class="<?php echo apply_filters( 'wpo_wcpdf_item_row_class', 'item-' . $item_id, esc_attr( $this->get_type() ), $this->order, $item_id ); ?>">
				<td class="product">
					...
					<div class="item-meta meta">
						...
						<!-- / .wc-item-meta -->
						<?php 
						if($order->get_meta( 'session_coach_id' )){
							$coach_name = get_user_meta($order->get_meta( 'session_coach_id' ), "fullname", true);
							echo "<b>Coach Name: </b>".$coach_name;
						}
						if($order->get_meta( 'session_date' )){
							$session_date = $order->get_meta( 'session_date' );
							echo "<br><b>Session Date & Time: </b>".date("m/d/Y h:i A", strtotime($session_date));
						} ?>
					</div>
